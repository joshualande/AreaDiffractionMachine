...
\begin{itemize}
    ...
    \item {\em \gui{Do Scale Factor} and \gui{Set Min/Max?}} -- Version 2 of
    the program introduced the feature \gui{Do Scale Factor?} and
    \gui{Set Min/Max?}. These can be used to override the minimum
    and maximum pixel value used by the Low and Hi sliders. The
    scale factor will simply divide the maximum intensity by this value.
    So if the intensity scale by default goes from 0 to 10000, a
    scale factor of 10 will make the scale instead go from 0 to 
    1000. The \gui{Set Min/Max?} option can instead be used to set
    directly the minimum and maximum intensity. This is a little confusing,
    but note that all this sets is the 
\end{itemize}
